...
object Benchmark {
  ...
  def withIO: ZIO[Any, Nothing, Unit] =
    for {
      v <- Random.nextLong
  ...
  def withoutIO: ZIO[Any, Nothing, Unit] =
    for {
      _ <- Random.nextLong
      _ <- Random.nextLong
      _ <- Random.nextLong
  ...
  def run(zio: ZIO[Any, Nothing, Unit], duration: Duration): ZIO[Any, Nothing, Unit] =
  ...
  private def iteration(zio: ZIO[Any, Nothing, Unit], stats: Ref[List[Long]]): ZIO[Any, Nothing, Unit] =
    for {
  ...
  private def printReport(stats: Ref[List[Long]], duration: Duration): ZIO[Any, Nothing, Unit] =
    for {
